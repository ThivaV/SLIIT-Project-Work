...
if (mString.length() > 0) {
    if (localName.equalsIgnoreCase("name")) {
            if (isPlacemark) {
            	...
            	String[] coodrinatesParsed = split(mString, " ");
				mRouteLength = mRoad.mRoute.length + coodrinatesParsed.length;
				double[][] mRouteOld = mRoad.mRoute;
				mRoad.mRoute = new double[mRouteLength][2];
				mRoad.mRoute = concat(mRouteOld, mRoad.mRoute);
				for (int i = 0; i < coodrinatesParsed.length; i++) {
					String[] xyParsed = split(coodrinatesParsed[i], ",");
					for (int j = 0; j < 2 && j < xyParsed.length; j++){
					mRoad.mRoute[lastIndex][j] = Double.parseDouble(xyParsed[j]);
					}
					lastIndex++;
				}
            }
...
public static double[][] concat(double[][] tabOld, double[][] tabNew) {
	for (int i = 0; i < tabOld.length; i++) {
		for (int j = 0; j < tabOld[i].length; j++) {
		tabNew[i][j] = tabOld[i][j];
		}
	}
	return tabNew;
}*/
